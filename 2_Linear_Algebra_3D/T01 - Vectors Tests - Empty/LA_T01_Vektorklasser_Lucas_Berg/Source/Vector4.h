...
namespace CommonUtilities
{
	...
	template <typename T>
	template <class TargetType>
	Vector4<TargetType> Vector4<T>::ToType() const
	{
		static_assert(std::is_constructible_v<TargetType, T> || std::is_convertible_v<T, TargetType>,
		              "Vector4::ToType element types are not convertible");
		return {
	...
	template <typename T>
	Tga::Vector4<T> Vector4<T>::ToTga() const
	{
	...
	inline T Vector4<T>::LengthSqr() const
	{
		long double s =
			...
			static_cast<long double>(w) * static_cast<long double>(w);
	...
	template <class T>
	inline bool IsSafeDivisor(const T& s)
	{
		if constexpr (std::is_floating_point_v<T>)
		{
	...
	inline Vector4<T> operator/(const Vector4<T>& aVector, const T& aScalar)
	{
		assert(IsSafeDivisor(aScalar) && "Vector4: divide by ~Zero");
		return {
			aVector.x / aScalar,
			aVector.y / aScalar,
			aVector.z / aScalar,
