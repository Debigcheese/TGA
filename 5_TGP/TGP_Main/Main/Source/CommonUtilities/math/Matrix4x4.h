...
namespace Tga
{
	...
	public:
		...
		void SetRotation(const Vector3<T>& aVector3); // Warning, this overwrites scale!
		//void SetScaleRotation(const Matrix3x3<T>& aMatrix);

		void NormalizeXYZ();
		Matrix4x4<T>& Translate(const Vector3<T>& aDirection);

		Matrix4x4<T>& ApplyTransform(Matrix4x4<T> transform)
		{
			*this = transform * *this;
			return *this;
		}
		...
		{
			*this = Matrix4x4<T>::CreateFromScale(scale) * *this;
			return *this;
		}

		Matrix4x4<T>& Scale(Vector3<T> scale)
		{
			*this = Matrix4x4<T>::CreateFromScale(scale) * *this;
			return *this;
		}

		Matrix4x4<T>& Rotate(Vector3<T> rotation)
		{
			*this = Matrix4x4<T>::CreateFromRollPitchYaw(rotation) * *this;
			return *this;
		}

		Matrix4x4<T>& Rotate(Quaternion<T> rotation)
		{
			*this = Matrix4x4<T>::CreateFromRotation(rotation) * *this;
			return *this;
		}
		...
		static Matrix4x4<T> CreateFromTranslation(Vector3<T> aTranslationVector);
		static Matrix4x4<T> CreateRotationAroundX(T aAngleInDegrees);
		static Matrix4x4<T> CreateRotationAroundY(T aAngleInDegrees);
		static Matrix4x4<T> CreateRotationAroundZ(T aAngleInDegrees);
		static Matrix4x4<T> CreateFromRollPitchYaw(Vector3<T> aPitchYawRollvector);
		static Matrix4x4<T> CreateFromScale(Vector3<T> aScalarVector);
		static Matrix4x4<T> CreateFromRotation(Quaternion<T> aQuat);
		static Matrix4x4<T> CreateLookAt(const Vector3<T>& aPosition, const Vector3<T>& aDirection, const Vector3<T>& aUp);
		...
		const T* GetDataPtr() const { return myData; }
	private:
...
#pragma warning( default :4201) // Nonstandard nameless struct/union.
	...
	{
		Vector4<T> result;
		result.X = (aMatrix(1, 1) * aVector.X) + (aMatrix(2, 1) * aVector.Y) + (aMatrix(3, 1) * aVector.Z) + (aMatrix(4, 1) * 1.f);
		result.Y = (aMatrix(1, 2) * aVector.X) + (aMatrix(2, 2) * aVector.Y) + (aMatrix(3, 2) * aVector.Z) + (aMatrix(4, 2) * 1.f);
		result.Z = (aMatrix(1, 3) * aVector.X) + (aMatrix(2, 3) * aVector.Y) + (aMatrix(3, 3) * aVector.Z) + (aMatrix(4, 3) * 1.f);
		result.W = (aMatrix(1, 4) * aVector.X) + (aMatrix(2, 4) * aVector.Y) + (aMatrix(3, 4) * aVector.Z) + (aMatrix(4, 4) * 1.f);
		return result;
	}
...
#pragma endregion Operators
	...
	template<typename T>
	inline Quaternion<T> Matrix4x4<T>::GetRotationAsQuaternion() const
	{
		Vector3f rowX = { (*this)(1,1),(*this)(1,2) ,(*this)(1,3) };
		Vector3f rowY = { (*this)(2,1),(*this)(2,2) ,(*this)(2,3) };
		Vector3f rowZ = { (*this)(3,1),(*this)(3,2) ,(*this)(3,3) };
	...
	inline void Matrix4x4<T>::DecomposeMatrix(Vector3<T>& aPosition, Quaternion<T>& aRotation, Vector3<T>& aScale) const
	{
		...
		Vector4f rowX = { mat(1,1),mat(1,2) ,mat(1,3) ,mat(1,4) };
		Vector4f rowY = { mat(2,1),mat(2,2) ,mat(2,3) ,mat(2,4) };
		Vector4f rowZ = { mat(3,1),mat(3,2) ,mat(3,3) ,mat(3,4) };

		aScale.X = rowX.Length();
		aScale.Y = rowY.Length();
		aScale.Z = rowZ.Length();

		rowX.Normalize();
		rowY.Normalize();
		rowZ.Normalize();

		aRotation = Quaternion<T>::CreateFromOrthonormalBasisVectors(rowX, rowY, rowZ);

		aPosition = mat.GetPosition();
	}
	...
	template<typename T>
	Matrix4x4<T>& Matrix4x4<T>::Translate(const Vector3<T>& aDirection)
	{
...
#pragma region Static Functions
	...
	template<typename T>
	inline Matrix4x4<T> Matrix4x4<T>::CreateRotationAroundX(const T aAngleInDegrees)
	{
		const T cos = std::cos(FMath::DegToRad * aAngleInDegrees);
		const T sin = std::sin(FMath::DegToRad * aAngleInDegrees);
		return
		{
			T(1), 0,  0,  0,
			0,  cos, sin, 0,
	...
	template<typename T>
	inline Matrix4x4<T> Matrix4x4<T>::CreateRotationAroundY(const T aAngleInDegrees)
	{
		const T cos = std::cos(FMath::DegToRad * aAngleInDegrees);
		const T sin = std::sin(FMath::DegToRad * aAngleInDegrees);
		return
	...
	template<typename T>
	inline Matrix4x4<T> Matrix4x4<T>::CreateRotationAroundZ(const T aAngleInDegrees)
	{
		const T cos = std::cos(FMath::DegToRad * aAngleInDegrees);
		const T sin = std::sin(FMath::DegToRad * aAngleInDegrees);
		return
	...
	template<typename T>
	inline Matrix4x4<T> Matrix4x4<T>::CreateFromScale(Vector3<T> aScaleVector)
	{
		Matrix4x4<T> result;
	...
	template<typename T>
	inline Matrix4x4<T> Matrix4x4<T>::CreateFromRollPitchYaw(Vector3<T> aYawPitchRollvector)
	{
		Quaternion<T> rotation = Quaternion<T>(aYawPitchRollvector);
		return CreateFromRotation(rotation);
	}

	template<class T>
	inline Matrix4x4<T> Matrix4x4<T>::CreateFromRotation(Quaternion<T> aQuat)
	{
		Matrix4x4<T> result;
	...
	template<typename T>
	inline Matrix4x4<T> Matrix4x4<T>::CreateFromTranslation(Vector3<T> aTranslationVector)
	{
		Matrix4x4<T> result;
	...
	template<class T>
	inline Matrix4x4<T> Matrix4x4<T>::CreateLookAt(const Vector3<T>& aPosition, const Vector3<T>& aDirection, const Vector3<T>& aUp)
	{
		Vector3<T> xAxis = aUp.Cross(aDirection).GetNormalized();
		Vector3<T> yAxis = aDirection.Cross(xAxis);

		Matrix4x4<T> result;
		result.myXAxis = xAxis;
		result.myYAxis = yAxis;
		result.myZAxis = aDirection;
		result.myWAxis = aPosition;
	...
	{
		Matrix4x4f newRot = CreateFromRollPitchYaw(aVector3);
		this->myXAxis = newRot.myXAxis;
		this->myYAxis = newRot.myYAxis;
		this->myZAxis = newRot.myZAxis;
	}

	template<class T>
	inline void Matrix4x4<T>::ResetScaleAndRotation()
	{
		this->myXAxis = { 1, 0, 0 };
		this->myYAxis = { 0, 1, 0 };
		this->myZAxis = { 0, 0, 1 };
...
#define VecShuffle_2323(vec1, vec2)        _mm_movehl_ps(vec2, vec1)
	...
	template <typename T>
	inline Matrix4x4<float> Matrix4x4<T>::InverseFloat(const Matrix4x4<float>& aMatrixToInverse)
	{
	...
	template <typename T>
	inline Matrix4x4<float> Matrix4x4<T>::InverseFastFloat(const Matrix4x4<float>& aMatrixToInverse)
	{
		Matrix4x4<float> m;
