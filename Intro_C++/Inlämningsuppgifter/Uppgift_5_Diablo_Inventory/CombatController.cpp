...
void CombatController::UpdateCombat(Room* aCurrentRoom)
{
    ...
    while (true && !myPlayer.IsDead())
    {
        ...
        if (targetEnemy.IsDead())
        {
            ...
            ConsoleUI::PrintEnemies(myCurrentRoom);
        }
        else
        {
            ConsoleUI::PrintEnemiesWithTarget(GetAttackData().targetIndex, myCurrentRoom);
        }
...
float CombatController::GetDamageFromAttackType() const
{
    ...
    switch (GetAttackData().attackType)
    {
...
AmountRange CombatController::GetHeavyDamageRange() const
{
