...
namespace CommonUtilities
{
	...
	{
		static_assert(std::is_constructible_v<TargetType, T>
		              || std::is_convertible_v<T, TargetType>,
		              "Vector3::ToType element types are not convertible");
	...
	{
		return {static_cast<long double>(x) * x + static_cast<long double>(y) * y + static_cast<long double>(z) * z};
	}

	template <typename T>
	inline T Vector3<T>::Length() const
	{
		return {static_cast<long double>(std::sqrt(LengthSqr()))};
	}

	template <typename T>
	Vector3<T> Vector3<T>::GetNormalized() const
	{
		// std::is_floating_point_v<T> -> a bool, is true if T is float, double, or long double; otherwise false.
		// std::conditional_t<Cond, A, B> -> picks a type : if Cond is true, the result type is A; else B.
		// std::numeric_limits<R>::epsilon() = treat as zero
		...
		return Vector3<R>{static_cast<R>(x) / length, static_cast<R>(y) / length, static_cast<R>(z)};
	}
	...
	void Vector3<T>::Normalize()
	{
		...
		return Vector3<R>{static_cast<R>(x) / length, static_cast<R>(y) / length, static_cast<R>(z) / length};
	}
	...
	Vector3<T> operator/(const Vector3<T>& aVector, const T& aScalar)
	{
		...
		{
			assert(std::fabs(aScalar) > std::numeric_limits<T>::epsilon() &&
				"Vector3: divide by ~zero");
		}
		else
		{
			assert(aScalar != 0 && "Vector3: divide by zero");
		}
