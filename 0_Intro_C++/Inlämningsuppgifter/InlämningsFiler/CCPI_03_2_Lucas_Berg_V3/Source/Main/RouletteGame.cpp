...
void RouletteGame::DrawRouletteBoard() const
{
	for (int i = 0; i < ROULETTE_ARRAY_SIZE; i++)
	{
		...
		{
			if ((i % myColumnAmount == static_cast<int>(Columns::Left)) && i <
				myBoardSingleDigitThreshold)
			{
				std::cout << " ";
			}
...
int RouletteGame::GetRoulettePayoutAmount() const
{
	...
	switch (myRoulette.betType)
	{
		case RouletteBetType::Straight:
		{
			payoutAmount = myBetMultiStraight;
			break;
		}
		case RouletteBetType::RedBlack:
		{
			payoutAmount = myBetMultiColor;
			break;
		}
		case RouletteBetType::OddEven:
		{
			payoutAmount = myBetMultiOddOrEven;
			break;
		}
		case RouletteBetType::Column:
		{
			payoutAmount = myBetMultiRoulette;
			break;
		}
		default:
		{
...
Columns RouletteGame::GetColumnFromIndex(const int aIndex) const
{
	...
	if (aIndex % myColumnAmount == myColumnLeftRemainder)
	{
		return Columns::Left;
	}
	if (aIndex % myColumnAmount == myColumnMiddleRemainder)
	{
